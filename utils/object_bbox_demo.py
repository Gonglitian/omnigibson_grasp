...
def get_object_bbox_before_loading(category, model):
    ...
    usd_path = os.path.join(gm.DATASET_PATH, "objects", category, model, "usd", f"{model}.usd")
...
def get_available_categories():
    ...
    categories = [d for d in os.listdir(object_dir) if os.path.isdir(os.path.join(object_dir, d))]
...
def get_models_for_category(category):
    ...
    models = [d for d in os.listdir(category_dir) if os.path.isdir(os.path.join(category_dir, d))]
...
def compare_categories(categories, max_models=3):
    ...
    print(f"比较 {', '.join(categories)} 类别的体积分布 (每类最多 {max_models} 个模型)...")
    
    # 获取所有类别的体积数据
    category_volumes = {}
    for category in categories:
        results = batch_get_volume_for_category(category, max_models)
        volumes = [data["volume"] for data in results.values() if data["volume"] is not None]
        if volumes:
    ...
    # 计算统计信息
    print("\n体积统计信息:")
    print(f"{'类别':<20} {'平均体积':<15} {'最小体积':<15} {'最大体积':<15} {'样本数':<10}")
    print("-" * 75)
    ...
    for category, volumes in category_volumes.items():
        ...
        min_vol = min(volumes)
        max_vol = max(volumes)
        print(f"{category:<20} {avg:.6f}{'':8} {min_vol:.6f}{'':8} {max_vol:.6f}{'':8} {len(volumes):<10}")
    
    # 绘制箱线图比较
    plt.figure(figsize=(12, 8))
    plt.boxplot([volumes for volumes in category_volumes.values()], 
...
def main():
    ...
    while True:
        ...
        if choice == "0":
            ...
            cat_indices = input("\n请选择要比较的类别编号（用逗号分隔，如1,3,5）: ").split(",")
            selected_categories = []
